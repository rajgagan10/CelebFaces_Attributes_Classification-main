...
def main():
    ...
    # ✅ handle GPU vs CPU for PL 2.x
    if getattr(config.hparams, "gpu", 0) and int(config.hparams.gpu) > 0:
        accelerator = "gpu"
        devices = int(config.hparams.gpu)
    else:
        accelerator = "cpu"
        devices = 1
    ...
    if config.hparams.train:
        ...
        callbacks = [
            EarlyStopping(**config.callback_param.early_stopping_params),
        ...
        trainer = Trainer(
            ...
            devices=devices,
            # auto_scale_batch_size="power",  # optional
            callbacks=callbacks,
            log_every_n_steps=1,
    ...
    if config.hparams.predict:
        ...
        predictions = trainer.predict(
            model, dataset_module, ckpt_path=config.inference_param.ckpt_path
        )
        ...
        for pred_batch in predictions:
            img_names, preds, converted_preds, converted_logits = (
                pred_batch[0],
                pred_batch[1],
                pred_batch[2],
                pred_batch[3],
            ...
            for i, img_name in enumerate(img_names):
                ...
                preds_with_conf = {
                    ATTRIBUTES[idx]: round(
                        converted_logits.tolist()[i][idx], 3
                    )
                    for idx in np.where(preds[i] == 1.0)[0]
                }
